...
import { Pet, PetTypeOptions } from "../models/pet.interface";
import {
  StudentForm,
...
export class StudentFacadeService {
    ...
    return new FormGroup({
      ...
      personal_information: new FormGroup<StudentFormPersonalInformation>({
        ...
        age: new FormControl<number>(student?.age ?? 18, {
          nonNullable: true,
          validators: [Validators.required, Validators.min(18), Validators.max(70)],
        }),
      ...
      location: new FormGroup<StudentFormLocation>({
        ...
        postalCode: new FormControl<string>(student?.location?.postalCode ?? "", { nonNullable: true, validators: [Validators.required] }),
      ...
      hobbies: new FormArray(student?.hobbies.length ? student?.hobbies.map((hobby) => this.addHobby(hobby)) : [this.addHobby()]),
      knownLanguages: new FormArray(student?.knownLanguages.length ? student?.knownLanguages.map((language) => this.addHobby(language)) : [this.addLanguage()]),
      pets: new FormArray(student?.pets.length ? student?.pets.map((pet) => this.addPet(pet)) : [this.addPet()]),
      jobExperiences: new FormArray(
        student?.jobExperiences.length ? student?.jobExperiences.map((jobExperience) => this.addJobExperience(jobExperience)) : [this.addJobExperience()]
      ),
    });
  }
  ...
  addJobExperience(jobExperience?: StudentExperience): FormGroup<StudentFormJobExperience> {
    return new FormGroup<StudentFormJobExperience>({
      ...
      tags: new FormArray(jobExperience?.tags?.length ? jobExperience?.tags.map((tag) => this.addJobExperienceTag(tag)) : [this.addJobExperienceTag()]),
    });
  }
